...
from bundle import bundle, _next_version, _sha256_file, REGISTRY_PATH, BUNDLE_DIR
...
class TestScoreGate:
    def test_rejects_below_min_score(self, adapter_dir, cfg_path, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("bundle.REGISTRY_PATH", tmp_path / "bundles.json")
...
class TestVersioning:
    ...
    def test_increments_version(self, tmp_path, monkeypatch):
        ...
        reg_path = tmp_path / "bundles.json"
...
class TestRegistry:
    def test_registry_created_on_first_bundle(self, adapter_dir, cfg_path, tmp_path, monkeypatch):
        ...
        assert data["bundles"][0]["adapter"] == "test-adapter"
        assert data["bundles"][0]["version"] == 1
        assert data["bundles"][0]["score"] == 0.99
    ...
    def test_multiple_bundles_append(self, adapter_dir, cfg_path, tmp_path, monkeypatch):
        ...
        assert data["bundles"][1]["version"] == 2
